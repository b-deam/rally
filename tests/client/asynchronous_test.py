...
class TestResolveHost:
    ...
    async def test_resolve_host_even_client_allocation(
        ...
        """
        TCPConnector._resolve_host() returns all the IPs a given name resolves to, but the underlying connection
        logic in TCPConnector._create_direct_connection() only ever selects the first succesful host from this list.

        So, we use the factory assigned client_id to deterministically return a specific IP which we replace at the
        beginning of the list to evenly distributes connections across _all_ clients.
